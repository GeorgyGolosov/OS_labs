...
void* monte_carlo(void* arg) {
    ...
    unsigned int thread_id = time(NULL) ^ pthread_self();
    // unsigned int thread_id = rand(); // Уникальный thread_id для каждого потока
    for (int i = 0; i < data->points_per_thread; ++i) {
        double x = (double)rand_r(&thread_id) / RAND_MAX;
        double y = (double)rand_r(&thread_id) / RAND_MAX;
        if (x * x + y * y <= 1.0) {
            inside_circle++;
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    int points_per_thread = 1000000000 / threads_number;
